...
- 圆形网页悬浮按钮：单击翻译、再次单击还原，悬浮展开模型与设置，拖动松手后自动贴边
...
扩展翻译视频页面已经提供的字幕，包括浏览器原生字幕轨和常见播放器渲染的字幕 DOM。译文会贴合原生字幕的底部位置，并严格裁切在视频画面内。它不会绕过 DRM，也不会偷偷上传整段音轨。没有字幕轨的视频不会自动变成语音识别字幕；如需该能力，应另行接入流式语音转写服务，并先确认平台条款、版权、隐私与费用。
